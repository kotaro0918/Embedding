...
## 内部での動き
このレポジトリでは、各形式のファイルを分割しEmbeddingしたものをDbに保存することで簡易的なRAGを行なっています  

```
Dbにベクトルを保存→プロンプトにコサイン類似度の高い文章を加えてLLMに入力
```

上のように動作することでRAGを実現しています
csv,pdf,txtの３種類でRAGを行い、その精度についても述べておいたのでぜひ読んでください


色々動かしていたらファイルの数が増えてしまったんですが、整頓するのが面倒なので頑張って探してください
...
まずはが分類項目基準をどのように分割するかが重要である。この時に注意するのが、分割一つあたり十分な情報量を 
持たせつつ、token数を超えるほどの情報は持たせないことにある。  
